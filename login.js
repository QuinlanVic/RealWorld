...
var $author$project$Login$initialModel = {bio: '', email: '', errmsg: '', image: '', loggedIn: false, password: '', token: '', username: ''};
...
var $author$project$Login$LoadUser = function (a) {
	return {$: 'LoadUser', a: a};
};
var $author$project$Login$baseUrl = 'http://127.0.0.1:8010/proxy/';
...
var $author$project$Login$encodeUser = function (user) {
...
var $author$project$Login$User = F8(
	function (email, token, username, bio, image, password, loggedIn, errmsg) {
		return {bio: bio, email: email, errmsg: errmsg, image: image, loggedIn: loggedIn, password: password, token: token, username: username};
	});
...
var $author$project$Login$userDecoder = A2(
					...
					A3(
						...
						A3(
							...
							A3(
								...
								'email',
								$elm$json$Json$Decode$string,
								$elm$json$Json$Decode$succeed($author$project$Login$User)))))))));
var $author$project$Login$saveUser = function (user) {
	var body = $elm$http$Http$jsonBody(
		$author$project$Login$encodeUser(user));
	return $elm$http$Http$post(
		{
			body: body,
			expect: A2($elm$http$Http$expectJson, $author$project$Login$LoadUser, $author$project$Login$userDecoder),
			url: $author$project$Login$baseUrl + 'api/users/login'
		});
};
			...
			case 'Login':
				return _Utils_Tuple2(
					_Utils_update(
						user,
						{loggedIn: true}),
					$author$project$Login$saveUser(user));
			default:
				var result = message.a;
				return A2($author$project$Login$getUserCompleted, user, result);
		}
	});
